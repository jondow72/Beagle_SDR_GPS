...
function kiwi_queue_or_camp_cb(path,val,first){var url=window.location.href;console.log(url);url=url+kiwi_add_search_param(window.location,'camp');console.log('--> '+url);kiwi_open_or_reload_page({url:url});}
...
kiwi_storeInit('freq_dsp_1Hz',cfg.show_1Hz?1:0);w3_innerHTML('rx-antenna','Antenna: '+ext_get_cfg_param_string('rx_antenna'));kiwi.WSPR_rgrid=ext_get_cfg_param_string('WSPR.grid','',EXT_NO_SAVE);}
...
function kiwi_ip_limit_pwd_cb(pwd){console.log('kiwi_ip_limit_pwd_cb pwd='+pwd);kiwi_storeWrite('iplimit',encodeURIComponent(pwd));window.location.reload(true);}
...
s=reason;}
kiwi_show_msg(s);}
var stats_interval=10000;function stats_init(){msg_send('SET GET_CONFIG');stats_update();}
...
break;case"load_adm":var adm_json=decodeURIComponent(param[1]);console.log('### load_adm '+ws.stream+' '+adm_json.length);adm=kiwi_JSON_parse('load_adm',adm_json);break;case"cfg_loaded":console.log('### cfg_loaded');owrx_init_cfg();w3_call('ant_switch_user_refresh');break;case"no_admin_conns":var retry_dx_update=parseInt(param[1]);kiwi.no_admin_conns_pend++;if(kiwi.no_admin_conns_pend==1){confirmation_panel_close();confirmation_show_content('Must close admin connection before attempting this operation.'+
...
kiwi_snr_stats(o.sa,o.sh);admin_stats_cb(o.ad,o.au,o.ae,o.ar,o.an,o.ap,o.an2,o.ai);w3_call('config_status_cb',o);time_display_cb(o);}
break;case"output_msg":kiwi_output_msg_p.s=param[1];kiwi_output_msg('id-output-msg','id-output-msg',kiwi_output_msg_p);break;case"status_msg_html":var s=w3_json_to_html('status_msg_html',param[1]);w3_innerHTML('id-status-msg',s);w3_innerHTML('id-msg-status',s);break;case"is_admin":extint.isAdmin_cb(param[1]);break;case"is_local":var p=param[1].split(',');console.log('kiwi_msg rx_chan='+p[0]+' is_local='+p[1]);kiwi.is_local[+p[0]]=+p[1];kiwi.tlimit_exempt_by_pwd[+p[0]]=+p[2];break;case"no_reopen_retry":kiwi.no_reopen_retry=true;break;case"is_multi_core":kiwi.is_multi_core=1;break;case"authkey_cb":extint.authkey_cb(param[1]);break;case"down":kiwi_down(param[1],reason_disabled);break;case"too_busy":kiwi_too_busy(parseInt(param[1]));break;case"monitor":kiwi_monitor();break;case"exclusive_use":kiwi_exclusive_use();break;case"inactivity_timeout":kiwi_inactivity_timeout(param[1]);break;case"ip_limit":var p=decodeURIComponent(param[1]).split(',');kiwi_24hr_ip_limit(parseInt(p[0]),p[1]);break;case"password_timeout":kiwi_password_entry_timeout();break;case"reason_disabled":reason_disabled='User connections disabled.'+
((param[1]!='')?('<br>Reason: '+w3_json_to_html('reason_disabled',param[1])):'');break;case"sample_rate":extint.srate=parseFloat(param[1]);break;case'pref_import_ch':kiwi.prefs_import_ch=+param[1];break;case'pref_import':prefs_import_cb(param[1],kiwi.prefs_import_ch);break;case'adc_clk_nom':extint.adc_clock_nom_Hz=+param[1];break;case'notify_msg':var s=w3_json_to_html('notify_msg',param[1]);console.log('notify_msg: '+s);if(confirmation.displayed)break;s=w3_div('',s);confirmation_show_content(s,425,50);setTimeout(confirmation_panel_close,3000);break;case'kiwi_kick':var p=param[1].split(',');var kicked=+p[0];var s=ext_get_cfg_param_string(kicked?'cfg.reason_kicked':'cfg.reason_disabled');var s=w3_json_to_html('kiwi_kick',p[1])+((s!='')?('<br>Reason: '+s):'');if(confirmation.displayed)break;s=w3_div('',s);confirmation_show_content(s,425,75,null,'red');break;case'snr_stats':var p=param[1].split(',');kiwi_snr_stats(p[0],p[1]);break;default:if(param[0].startsWith('antsw_')){if(ant_switch_msg(param))
break;}
rtn=false;break;}
return rtn;}
